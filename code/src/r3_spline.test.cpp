...
TEST(r3Spline, Testr3SplineInvalidEvaluateConditions) {
    ...
    EXPECT_EQ(r3_spline.Evaluate(115, 0), std::nullopt);
    ...
    EXPECT_NE(r3_spline.Evaluate(100, 0), std::nullopt);  // Inside first time segment - valid
    EXPECT_EQ(r3_spline.Evaluate(105, 0), std::nullopt);  // Outside first time segment - invalid

    // Add one more knot to see that we can now do a valid evaluation in the second time segment
    r3_spline.knots_.push_back(VectorD::Zero());
    EXPECT_NE(r3_spline.Evaluate(105, 0), std::nullopt);
}
...
TEST(r3Spline, Testr3SplineEvaluate) {
    ...
    auto const p_5{r3_spline.Evaluate(105, 0)};
...
TEST(r3Spline, Testr3SplineEvaluateDerivatives) {
    ...
    auto const p_du{r3_spline.Evaluate(101, 1)};
    ...
    auto const p_dudu{r3_spline.Evaluate(101, 2)};
